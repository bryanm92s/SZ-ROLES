...
// Fecha de creación en hora LOCAL (America/Bogota) como ISO sin 'Z'.
// A diferencia de `new Date().toISOString()` (que emite UTC y diferencia 5h),
// este produce 'YYYY-MM-DDTHH:mm:ss' con el reloj local del navegador.
// Coincide con el patrón que ya usa Code.gs para priceHistory.changedAt
// (Utilities.formatDate con Session.getScriptTimeZone), y ReportsTab.jsx
// ya sabe leerlo.
const localNowISO = () => {
  const d = new Date(), p = (n,a=2)=>String(n).padStart(a,'0')
...
const getSlots = (date, takenApptIds, allAppts, excludeId=null, assignedTo=null, clientPhone=null) => {
  const now     = new Date()
  const isToday = date === todayStr()
  // Normaliza celular: solo dígitos, para tolerar "322 399 2350" == "3223992350"
  const normPhone = p => String(p||'').replace(/\D/g,'')
  const cPhone = normPhone(clientPhone)
  // Toma los minutos ocupados por (a) la empleada asignada y (b) el cliente
  // (sin importar quién lo atienda). Regla de negocio: un cliente no puede
  // tener dos citas solapadas inclusive si lo atienden personas distintas.
  const taken = []   // {tMin, byClient}
  allAppts.forEach(a => {
    if (cleanDate(a.date) !== date) return
    if (a.id === excludeId) return
    const sameEmp = assignedTo
      && String(a.assignedTo||'').trim().toLowerCase() === String(assignedTo).trim().toLowerCase()
    const sameCli = cPhone
      && normPhone(a.clientPhone) === cPhone
    if (sameEmp) taken.push({ tMin: toMinutes(cleanTime(a.time)), byClient: false })
    else if (sameCli) taken.push({ tMin: toMinutes(cleanTime(a.time)), byClient: true })
  })
  ...
  return TIME_SLOTS.map(t => {
    ...
    const hit = taken.find(o => Math.abs(tMin - o.tMin) < SERVICE_DURATION)
    ...
    const isTaken = !!hit
    const reason = isPast ? 'Hora pasada'
                 : hit && hit.byClient ? 'Cliente ocupado'
                 : 'Ocupada'
    return { time:t, disabled: isTaken||isPast, reason }
  })
}

// Busca citas existentes que crucen con {date,time} por empleado O por cliente
// (mismo clientPhone), usando la misma regla de solapamiento de 60 min que
// getSlots. Aplica a admin y a empleada (corrige el match exacto anterior).
// Devuelve array de citas conflictivas (vacío si no hay cruce).
const findConflicts = (appts, opts) => {
  const {date, time, assignedTo, clientPhone, excludeId} = opts
  const t = toMinutes(cleanTime(time))
  if (!date || !time || isNaN(t)) return []
  const cPhone = clientPhone ? String(clientPhone).replace(/\D/g,'') : ''
  return appts.filter(a => {
    if (a.id === excludeId) return false
    if (cleanDate(a.date) !== cleanDate(date)) return false
    const s = toMinutes(cleanTime(a.time))
    if (isNaN(s) || Math.abs(t - s) >= SERVICE_DURATION) return false
    const sameEmp = assignedTo
      && String(a.assignedTo||'').trim().toLowerCase() === String(assignedTo).trim().toLowerCase()
    const sameCli = cPhone
      && String(a.clientPhone||'').replace(/\D/g,'') === cPhone
    return !!(sameEmp || sameCli)
  })
}
...
export default function App() {
  ...
  // Evitamos placeholders VITE_xxx en index.html porque Vite hace decodeURI()
  // sobre los href durante `vite build` y revienta con "URI malformed".
  // El <script> de index.html que antes hacia el swap por VITE_BIZ_LOGO se
  // elimina porque %VITE_*% en JS inline también cuelea warnings de build.
  useEffect(() => {
    ...
    if (BIZ_LOGO) {
      // Si hay logo configurado, reemplaza el favicon por la logo.
      let link = document.querySelector("link[rel~='icon']")
      if (!link) { link = document.createElement('link'); link.rel='icon'; document.head.appendChild(link) }
      link.href = BIZ_LOGO
  ...
  return (
    ...
    <div style={{fontFamily:"'DM Sans',system-ui,sans-serif",minHeight:'100vh',background:'var(--bg)',color:'var(--t)'}}>
      ...
      <nav style={{background:'var(--card)',borderBottom:'1px solid var(--border)',display:'flex',overflowX:'auto',padding:'0 8px',position:'sticky',top:52,zIndex:99,scrollbarWidth:'none',boxShadow:'0 2px 12px rgba(0,0,0,0.05)',WebkitOverflowScrolling:'touch'}}>
        {[
          ...
          <button key={id} onClick={()=>setTab(id)} className={`nb${tab===id?' act':''}`}
            style={{display:'flex',flexDirection:'column',alignItems:'center',gap:3,paddingTop:9,paddingBottom:9,paddingLeft:10,paddingRight:10,minWidth:'fit-content'}}>
...
function MyExpensesTab({expenses, visibleExpenses, SE, confirm, userEmail}) {
  ...
  const [amountErr,     setAmountErr]    = useState('') // "El monto del gasto no puede ser negativo."
  const [editAmountErr, setEditAmountErr]= useState('') // idem para edición
  ...
  const add = () => {
    ...
    if (parsed < 0) { setAmountErr('El monto del gasto no puede ser negativo.'); return }
    setAmountErr('')
    SE([...allExpenses, {id:uid(),description:capFirst(desc),amount:Math.max(0, parsed),category:capFirst(cat),date:expDate,createdBy:userEmail||''}])
    setD(''); setA('')
  }

  const saveEdit = () => {
    if (editData.amount!==undefined && Number(editData.amount) < 0) { setEditAmountErr('El monto del gasto no puede ser negativo.'); return }
    setEditAmountErr('')
    SE(allExpenses.map(e=>e.id===editId?{...e, amount: editData.amount!==undefined ? Math.max(0, toN(editData.amount)) : e.amount, ...editData,description:capFirst(editData.description||'')}:e))
    setEI(null)
  }
      ...
      <div className="card">
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:10}}>
          ...
          <div>
            ...
            <input className="inp" type="number" min="0" placeholder="20000" value={amount}
              onChange={e=>{const v=e.target.value; setA(v); setAmountErr(v!==''&&Number(v)<0?'El monto del gasto no puede ser negativo.':'')}}
              style={amountErr?{borderColor:'var(--red)'}:undefined}/>
            {amountErr && <div style={{fontSize:11,color:'var(--red)',marginTop:4,fontWeight:600}}>⚠️ {amountErr}</div>}
          </div>
          <div>
                ...
                {isEdit ? (
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                    ...
                    <div>
                      ...
                      <input className="inp" type="number" min="0" value={editData.amount||''}
                        onChange={ev=>{const v=ev.target.value; setED2(d=>({...d,amount:v})); setEditAmountErr(v!==''&&Number(v)<0?'El monto del gasto no puede ser negativo.':'')}}
                        style={editAmountErr?{borderColor:'var(--red)'}:undefined}/>
                      {editAmountErr && <div style={{fontSize:10,color:'var(--red)',marginTop:4,fontWeight:600,maxWidth:160}}>⚠️ {editAmountErr}</div>}
                    </div>
                    ...
                    <div style={{gridColumn:'span 2',display:'flex',gap:8}}>
                      <button className="btn" style={{flex:1}} disabled={!!editAmountErr} onClick={saveEdit}>Guardar</button>
                      <button className="btn-del" onClick={()=>{setEI(null);setEditAmountErr('')}}>Cancelar</button>
                    </div>
                  </div>
                ) : (
...
function GS() { return <style>{`
  ...
  :root{
    ...
    --warn-bg:#FFF4DC;--warn-t:#7A5000;
    /* Neumorphism shadows - MORE PRONOUNCED */
    --neumorph-shadow: 12px 12px 24px rgba(0,0,0,0.12), -12px -12px 24px rgba(255,255,255,0.8);
    --neumorph-shadow-inset: inset 6px 6px 12px rgba(0,0,0,0.1), inset -6px -6px 12px rgba(255,255,255,0.8);
    --neumorph-shadow-sm: 6px 6px 12px rgba(0,0,0,0.08), -6px -6px 12px rgba(255,255,255,0.8);
    --neumorph-shadow-lg: 20px 20px 40px rgba(0,0,0,0.15), -20px -20px 40px rgba(255,255,255,0.8);
    /* Glassmorphism */
    --glass-bg: rgba(255,255,255,0.75);
    --glass-border: rgba(255,255,255,0.4);
    --glass-blur: blur(20px);
    /* Modern flat */
    --flat-shadow: 0 4px 16px rgba(0,0,0,0.08);
    --flat-shadow-hover: 0 8px 24px rgba(0,0,0,0.12);
    --radius-sm: 8px;
    --radius-md: 12px;
    --radius-lg: 16px;
    --radius-xl: 24px;
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  body{-webkit-tap-highlight-color:transparent;background:var(--bg);color:var(--t);font-family:'DM Sans',system-ui,sans-serif;min-width:320px}
  ...
  .nb{background:none;border:none;border-bottom:2.5px solid transparent;padding:10px 8px;font-size:12px;font-weight:500;cursor:pointer;color:var(--t2);white-space:nowrap;font-family:inherit;transition:all .15s;flex-shrink:0;min-width:fit-content}
  .nb.act{color:var(--primary);border-bottom-color:var(--primary);font-weight:700}
  nav{background:var(--card);border-bottom:1px solid var(--border);display:flex;overflow-x:auto;padding:0 8px;position:sticky;top:58px;z-index:99;scrollbar-width:none;box-shadow:0 2px 12px rgba(0,0,0,0.05);-webkit-overflow-scrolling:touch}
  nav::-webkit-scrollbar{display:none}
  nav::after{content:'';flex-shrink:0;width:16px} /* spacing hint */
  
  /* Header - Mobile */
  header{background:var(--grad-header);padding:12px 16px;display:flex;align-items:center;justify-content:space-between;position:sticky;top:0;z-index:100;box-shadow:0 2px 20px rgba(0,0,0,0.15);flex-wrap:wrap;gap:8px}
  header > div:first-child{flex:1;min-width:0}
  header > div:last-child{display:flex;align-items:center;gap:6px;flex-shrink:0}
  
  /* ========================================
     DESIGN SYSTEM - ENHANCED VISIBILITY
  ======================================== */
  ...
  .inp:focus{border-color:var(--primary);box-shadow:0 0 0 4px var(--primary-l),var(--neumorph-shadow-inset);transform:scale(1.01)}
  .inp:disabled{background:var(--bg);color:var(--t2);cursor:not-allowed}
  .inp::placeholder{color:var(--t2);opacity:0.6}
  
  /* Labels */
  .lbl{display:block;font-size:11px;font-weight:700;color:var(--t2);text-transform:uppercase;letter-spacing:.08em;margin-bottom:6px}
  ...
  /* Tags */
  .tag{display:inline-flex;align-items:center;gap:4px;background:var(--primary-l);color:var(--primary);border-radius:20px;padding:4px 12px;font-size:11px;font-weight:700}
  .tag-g{display:inline-flex;align-items:center;gap:4px;background:#EDF7F0;color:var(--green);border-radius:20px;padding:4px 12px;font-size:11px;font-weight:700}
  .tag-gold{display:inline-flex;align-items:center;gap:4px;background:#FFF8E6;color:var(--gold);border-radius:20px;padding:4px 12px;font-size:11px;font-weight:700}
  .tag-past{display:inline-flex;align-items:center;gap:4px;background:var(--border);color:var(--t2);border-radius:20px;padding:4px 12px;font-size:11px;font-weight:700}
  ...
  /* Warning box */
  .warn-box{background:var(--warn-bg);border:1px solid #F0D870;border-radius:var(--radius-md);padding:12px 16px;margin-bottom:14px;font-size:13px;color:var(--warn-t)}
  ...
  @media (max-width: 768px) {
    ...
    .inp{padding:14px 14px;font-size:16px} /* prevent zoom on iOS */
    .btn{padding:14px 20px;font-size:15px}
    .btn-o{padding:12px 16px;font-size:14px}
    .stat{padding:16px 12px}
    .to{padding:8px 3px;font-size:10px;min-width:40px;min-height:40px}
  ...
  @media (max-width: 480px) {
    ...
    .lbl{font-size:10px}
    .tag,.tag-g,.tag-gold,.tag-past{font-size:10px;padding:3px 10px}
    .row{padding:10px 0}
  ...
  @keyframes remindPing{0%,100%{transform:scale(1)}50%{transform:scale(1.05)}}
  ...
  /* Focus visible */
  *:focus-visible{outline:3px solid var(--primary-l);outline-offset:2px}
  ...
  @media (prefers-contrast: high){
    :root{--border:#999;--t2:#333}
    .card,.card-neu,.card-glass{border-width:2px}
    .inp{border-width:2px}
    .btn-check{border-width:3px}
    .so{border-width:3px}
    .to{border-width:3px}
  }
`}</style> }
...
══════════════════════════════════════════════════════════════ */
/* ══════════════════════════════════════════════════════════════
   GESTIÓN DE ACCESOS (solo admin) — vive en la pestaña Config
══════════════════════════════════════════════════════════════ */
function AccessManager({ userEmail }) {
...
function SettingsTab({ paletteId, savePalette, SA, SC, SE, SS, sync, confirm, isAdmin, userEmail, setPH }) {
  ...
  return (
    <div style={{padding:'0 0 80px'}}>
      ...
      {/* ── GESTIÓN DE ACCESOS — solo Administradora ── */}
      {isAdmin && (
        <div className="card" style={{marginBottom:12}}>
          {sectionTitle('👥', 'Gestión de accesos')}
          ...
          <AccessManager userEmail={userEmail} />
        </div>
      )}
...
function Dashboard({clients,appts,visibleAppts,expenses,setTab,userEmail,isAdmin,userName}) {
  ...
  return <>
    ...
    {/* Alerta "Por recordar": citas pendientes que ocurren en la próxima hora
        (o hace menos de 10 min). Avisa a la operadora que es el momento ideal
        para enviar el recordatorio manual por WhatsApp. */}
    {(() => {
      const dispForRemind = isAdmin ? appts : (visibleAppts||appts)
      const toRemind = (Array.isArray(dispForRemind)?dispForRemind:[])
        .filter(a => !bool(a.completed) && a.completed!=='noshow')
        .map(a => {
          const d = cleanDate(a.date), t = cleanTime(a.time)
          if (!d || !t) return null
          const [h, m] = t.split(':').map(Number)
          const apt = new Date(`${d}T00:00:00`); apt.setHours(h, m, 0, 0)
          return { a, mins: Math.round((apt.getTime() - Date.now()) / 60000) }
        })
        .filter(Boolean)
        .filter(({mins}) => mins <= 60 && mins >= -10)
        .sort((x, y) => x.mins - y.mins)
      if (toRemind.length === 0) return null
      return <div style={{background:'#FFF4DC',border:'1.5px solid #F6C90E',borderRadius:12,padding:'12px 14px',marginBottom:14,display:'flex',gap:10,alignItems:'flex-start'}}>
        <span style={{fontSize:20,flexShrink:0}}>⏰</span>
        <div style={{flex:1}}>
          <div style={{fontWeight:700,fontSize:13,color:'#7A5000',marginBottom:6}}>
            {toRemind.length===1 ? 'Hay 1 cita para recordar ahora:' : `Hay ${toRemind.length} citas para recordar ahora:`}
          </div>
          {toRemind.map(({a, mins}) => {
            const dom = bool(a.domicilio)
            const attendee = (a.assignedTo||'').split('@')[0].replace(/[._]/g,' ').replace(/\b\w/g,c=>c.toUpperCase())
            const when = mins < 0 ? 'hace ' + Math.abs(mins) + ' min' : mins === 0 ? 'ahora' : 'en ' + mins + ' min'
            return <div key={a.id} style={{display:'flex',alignItems:'center',gap:6,marginBottom:6,flexWrap:'wrap',fontSize:12}}>
              <span style={{color:'#7A5000'}}>
                <strong>{a.clientName}</strong> · {fmtTime(a.time)} ({when}) · {fmtM(a.totalPrice||a.servicePrice)}{dom?` · 🛵 ${fmtM(a.domicilioPrice)}`:''}{attendee?` · 👩‍💼 ${attendee}`:''}
              </span>
              <button className="btn-wa" style={{padding:'5px 10px',fontSize:12}}
                onClick={()=>openWA(a.clientPhone, a.clientName, a.time, a.date, a.serviceNames, a.totalPrice||a.servicePrice, dom, attendee)}>
                💬 Recordar
              </button>
            </div>
          })}
...
function ApptsTab({clients,services,appts,visibleAppts,SA,SC,sync,deleteAppt,confirm,infoModal,userEmail,isAdmin,userName,users,userNameMap,tabExtra,setTab,priceHistory}) {
  ...
  if (editAppt) return <EditAppt   appt={editAppt} services={services} appts={appts} SA={SA} sync={sync} infoModal={infoModal} onClose={()=>setEdit(null)} isAdmin={isAdmin} userEmail={userEmail} users={users} userNameMap={userNameMap} priceHistory={priceHistory}/>
...
function ApptCard({appt,canEdit,onToggle,onEdit,onDelete,userNameMap={}}) {
  ...
  // Cita futura: agendada para un día posterior a hoy (no se puede completar
  // ni marcar no-asistio hasta que llegue el día/la hora).
  const todayStrAppt = todayStr()
  const isFuture = status==='pending' && cleanDate(appt.date) > todayStrAppt
  // Recordatorio "Recordar ahora": cita pendiente dentro de la próxima hora
  // (o hace menos de 10 min — para admitir ligeras demoras). Calculamos en min
  // usando combinación de fecha+hora local, sin dependencias externas.
  const minsToAppt = (() => {
    const d = cleanDate(appt.date), t = cleanTime(appt.time)
    if (!d || !t) return Infinity
    const [h, m] = t.split(':').map(Number)
    const apt = new Date(`${d}T00:00:00`); apt.setHours(h, m, 0, 0)
    return Math.round((apt.getTime() - Date.now()) / 60000)
  })()
  const remindNow = status==='pending' && minsToAppt <= 60 && minsToAppt >= -10
  ...
  return (
    <div style={{background:bgMap[status],borderRadius:12,border:`1.5px solid ${brdMap[status]}`,borderLeft:`4px solid ${brdMap[status]}`,padding:14,marginTop:8}}>
      ...
      <div style={{display:'flex',gap:5,flexWrap:'wrap',marginBottom:8}}>
        ...
        {status==='pending'&& isFuture && <span style={{display:'inline-block',background:'#E8F0FE',color:'#1A56A0',borderRadius:20,padding:'2px 10px',fontSize:12,fontWeight:600}}>📅 Cita futura</span>}
        {status==='pending'&& past && !isFuture && <span className="tag-past" style={{fontSize:11}}>● Pasada</span>}
        {remindNow && (
          <span style={{display:'inline-block',background:'#FFF4DC',color:'#7A5000',borderRadius:20,padding:'2px 10px',fontSize:12,fontWeight:700,animation:'remindPing 1.2s ease-in-out infinite'}}>
            ⏰ Recordar ahora
          </span>
        )}
      </div>
      ...
      <div style={{display:'flex',gap:6,flexWrap:'wrap'}}>
        {isFuture && (
          <div style={{flexBasis:'100%',fontSize:11,color:'#1A56A0',background:'#E8F0FE',borderRadius:8,padding:'6px 10px',marginBottom:4,fontWeight:600}}>
            📅 Cita futura — no se puede completar aún
          </div>
        )}
        <button className={`btn-check${status==='done'?' done':''}`} onClick={()=>!isFuture&&onToggle('done')}
          disabled={isFuture}
          style={status==='done'?{}:{opacity: isFuture?.4:(status==='noshow'?.5:1), cursor: isFuture?'not-allowed':'pointer'}}>
          {status==='done'?'✓ Completada':'✓ Completada'}
        </button>
        <button onClick={()=>!isFuture&&onToggle('noshow')}
          disabled={isFuture}
          style={{background:status==='noshow'?'var(--red)':'var(--primary-l)',color:status==='noshow'?'white':'var(--red)',border:`1.5px solid ${status==='noshow'?'var(--red)':'var(--border)'}`,borderRadius:8,padding:'6px 10px',fontSize:12,fontWeight:600,cursor:isFuture?'not-allowed':'pointer',fontFamily:'inherit',transition:'all .15s',opacity: isFuture?.4:(status==='done'?.5:1)}}>
          ✗ No asistió
        </button>
...
function EditAppt({appt,services,appts,SA,sync,onClose,isAdmin,userEmail,users,userNameMap,priceHistory,infoModal}) {
  ...
  const slots    = getSlots(date, [], appts, appt.id, isAdmin ? assignedTo : null, appt.clientPhone||'')
  ...
  // Conflicto de horario (empleado O cliente) con solapamiento de 60 min.
  // Aplica a admin y a empleada (corrige el match exacto anterior, que se
  // saltaba superposiciones tipo 14:00 + 14:30 y no cubría a las empleadas).
  const safeAllAppts = Array.isArray(appts) ? appts : []
  const conflicts = findConflicts(safeAllAppts, {
    date, time,
  ...
  const conflictName = conflict
    ? ((userNameMap||{})[String(conflict.assignedTo).trim().toLowerCase()] || String(conflict.assignedTo).split('@')[0].replace(/[._]/g,' ').replace(/\b\w/g,c=>c.toUpperCase()))
    : ''
  // ¿El cruce es por el cliente (mismo celular, otra persona lo atiende)?
  const normPhone = p => String(p||'').replace(/\D/g,'')
  const conflictByClient = conflict && !!appt.clientPhone
    && normPhone(conflict.clientPhone) === normPhone(appt.clientPhone)
    && !(isAdmin && assignedTo && String(conflict.assignedTo||'').trim().toLowerCase() === String(assignedTo).trim().toLowerCase())
  ...
  const save = async () => {
    ...
    const r0 = await sync({appointments:next},null,null)
    // Red de seguridad: si el backend rechazó (p.ej. conflicto de cliente que
    // se coló), no persistir el optimismo local → no quedaría appt fantasma.
    if (r0 === null) {
      infoModal('No se pudo guardar la cita. Es posible que el horario choque con otra cita del mismo cliente.')
      setL(false)
      return
    }
    SA(next)
    ...
    <div className="card">
      ...
      <div style={{marginTop:4,background:'#FFFAF0',border:'1.5px solid #F0D898',borderRadius:12,padding:12,marginBottom:14}}>
        ...
        {dom && <div style={{marginTop:10}}>
          ...
          <input className="inp" placeholder="Dirección" value={addr} onChange={e=>setAddr(e.target.value)}
            style={dom&&!addr.trim()?{borderColor:'var(--red)'}:undefined}/>
          {dom && !addr.trim() && <div style={{fontSize:11,color:'var(--red)',marginTop:5,fontWeight:600}}>⚠️ La dirección es obligatoria cuando el servicio es a domicilio.</div>}
        </div>}
      </div>
      ...
      {conflict && (
        <div style={{background:'#FFF8E1',border:'1.5px solid #F6C90E',borderRadius:12,padding:'12px 14px',marginBottom:14,display:'flex',gap:10,alignItems:'flex-start'}}>
          ...
          <div style={{fontSize:13,color:'#7A5C00',lineHeight:1.5}}>
            <strong>Conflicto de horario:</strong> el día <strong>{fmtDate(date)}</strong> a las <strong>{fmtTime(time)}</strong>,{' '}
            {conflictByClient
              ? <>el cliente <strong>{conflict.clientName}</strong> ya tiene una cita a las <strong>{fmtTime(conflict.time)}</strong>{conflict.assignedTo ? <> con <strong>{conflictName}</strong></> : null}. Un mismo cliente no puede tener dos citas que se crucen, aunque lo atiendan personas diferentes.</>
              : <><strong>{conflictName}</strong> ya tiene una cita con <strong>{conflict.clientName}</strong> a las <strong>{fmtTime(conflict.time)}</strong>.</>}
          </div>
        </div>
      )}
      ...
      {result!==null
        ...
        : <div style={{display:'flex',gap:8}}>
            ...
            <button className="btn" style={{flex:1}} onClick={save} disabled={!time||svcIds.length===0||loading||!!conflicts.length||(dom&&!addr.trim())}>{loading?'⏳ Guardando…':'Guardar cambios'}</button>
          </div>
      }
    </div>
  </div>
...
function NewWizard({clients,services,appts,SA,SC,sync,infoModal,onClose,userEmail,isAdmin,userName,users,userNameMap}) {
  ...
  const grand     = svcTotal+(dom?toN(domP):0)
  const slots     = getSlots(date,[],appts,null, isAdmin ? assignedTo : (userEmail||null), fc?.phone||'')

  // Conflicto de horario (empleado O cliente) con solapamiento de 60 min.
  // Aplica a admin y empleada; corrige el match exacto anterior.
  const safeAllAppts = Array.isArray(appts) ? appts : []
  const conflictsNW = findConflicts(safeAllAppts, {
    date, time,
    assignedTo: isAdmin ? assignedTo : (userEmail||''),
    clientPhone: fc?.phone,
    excludeId: null,
  })
  const conflictNW = conflictsNW[0] || null
  const conflictNWName = conflictNW
    ? ((userNameMap||{})[String(conflictNW.assignedTo).trim().toLowerCase()] || String(conflictNW.assignedTo).split('@')[0].replace(/[._]/g,' ').replace(/\b\w/g,c=>c.toUpperCase()))
    : ''
  const normPhoneNW = p => String(p||'').replace(/\D/g,'')
  const conflictByClientNW = conflictNW && !!fc?.phone
    && normPhoneNW(conflictNW.clientPhone) === normPhoneNW(fc.phone)
    && !(isAdmin && assignedTo && String(conflictNW.assignedTo||'').trim().toLowerCase() === String(assignedTo).trim().toLowerCase())
  ...
  const confirm = async () => {
    ...
    // Red de seguridad: si el backend rechazó (p.ej. conflicto de cliente),
    // no persistir el optimismo local → no quedaría appt fantasma.
    if (res === null) {
      infoModal('No se pudo agendar la cita. Es posible que el horario choque con otra cita del mismo cliente.')
      setL(false)
      return
    }
    if (res?.calResult?.ok) {
        ...
        {conflictNW && (
          <div style={{background:'#FFF8E1',border:'1.5px solid #F6C90E',borderRadius:12,padding:'12px 14px',marginBottom:14,display:'flex',gap:10,alignItems:'flex-start'}}>
            ...
            <div style={{fontSize:13,color:'#7A5C00',lineHeight:1.5}}>
              <strong>Conflicto de horario:</strong> el día <strong>{fmtDate(date)}</strong> a las <strong>{fmtTime(time)}</strong>,{' '}
              {conflictByClientNW
                ? <>el cliente <strong>{conflictNW.clientName}</strong> ya tiene una cita a las <strong>{fmtTime(conflictNW.time)}</strong>{conflictNW.assignedTo ? <> con <strong>{conflictNWName}</strong></> : null}. Un mismo cliente no puede tener dos citas que se crucen, aunque lo atiendan personas diferentes.</>
                : <><strong>{conflictNWName}</strong> ya tiene una cita con <strong>{conflictNW.clientName}</strong> a las <strong>{fmtTime(conflictNW.time)}</strong>.</>}
            </div>
          </div>
        )}
        <button className="btn" style={{flex:1}} onClick={()=>setStep(4.5)} disabled={!time||!!conflictsNW.length}>Ver resumen</button>
      </div>
    </div>}
...
function ServicesTab({services,SS,confirm}) {
  ...
  const [priceErr, setPriceErr] = useState('') // "El precio del servicio no puede ser negativo."
  const [ePErr,     setEPErr]    = useState('') // idem para la edición inline
  const safe=Array.isArray(services)?services:[]
  const add=()=>{
    if(!name.trim()||!price)return
    const parsed = Number(price)
    if (parsed < 0) { setPriceErr('El precio del servicio no puede ser negativo.'); return }
    setPriceErr('')
    SS([...safe,{id:uid(),name:name.trim(),price:Math.max(0, parsed)}]);setN('');setP('')
  }
  return <>
    ...
    <div className="card">
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:12}}>
        ...
        <div>
          <label className="lbl">Precio (COP)</label><input className="inp" type="number" min="0" placeholder="35000" value={price}
            onChange={e=>{const v=e.target.value; setP(v); setPriceErr(v!==''&&Number(v)<0?'El precio del servicio no puede ser negativo.':'')}}
            style={priceErr?{borderColor:'var(--red)'}:undefined}/>
          {priceErr && <div style={{fontSize:11,color:'var(--red)',marginTop:4,fontWeight:600}}>⚠️ {priceErr}</div>}
        </div>
      </div>
        ...
        :safe.map(s=><div key={s.id} className="row">
          ...
          {editId===s.id
            ?<div style={{display:'flex',alignItems:'flex-start',gap:6}}>
              <div>
              <input className="inp" type="number" min="0" value={eP} onChange={e=>{const v=e.target.value; setEP(v); setEPErr(v!==''&&Number(v)<0?'El precio del servicio no puede ser negativo.':'')}}
                style={{width:100,padding:'6px 10px',fontSize:13,borderColor:ePErr?'var(--red)':'var(--border)'}}/>
              {ePErr && <div style={{fontSize:10,color:'var(--red)',marginTop:3,fontWeight:600,maxWidth:140}}>{ePErr}</div>}
              </div>
              <button className="btn" style={{padding:'6px 12px',fontSize:13}} disabled={!!ePErr} onClick={()=>{SS(safe.map(x=>x.id===s.id?{...x,price:Math.max(0,Number(eP))}:x));setEI(null);setEPErr('')}}>✓</button>
              <button className="btn-del" onClick={()=>{setEI(null);setEPErr('')}}>✕</button>
            </div>
...
function FinancesTab({appts,expenses,SE,setTab,confirm,userEmail,userNameMap={}}) {
  ...
  const add=()=>{
    ...
    if (parsed < 0) { setAmountErr('El monto del gasto no puede ser negativo.'); return }
    setAmountErr('')
    SE([...safe,{id:uid(),description:capFirst(desc),amount:Math.max(0, parsed),category:capFirst(customCat||cat),date:expDate,createdBy:userEmail||''}])
    setD('');setA('');setCC('')
  }
  const saveEdit=()=>{
    if (editData.amount!==undefined && Number(editData.amount) < 0) { setEditAmountErr('El monto del gasto no puede ser negativo.'); return }
    setEditAmountErr('')
    SE(safe.map(e=>e.id===editId?{...e, amount: editData.amount!==undefined ? Math.max(0, toN(editData.amount)) : e.amount, ...editData}:e));setEI(null)
  }
            ...
            {visibleCats.map(catName => {
              ...
              return (
                <span key={catName} style={{display:'inline-flex',alignItems:'center',gap:4,padding:'3px 10px',borderRadius:20,background:'var(--primary-l)',border:'1px solid var(--border)',fontSize:12}}>
                  ...
                  {!inUse && (
                    <button onClick={()=>confirm(`¿Eliminar la categoría "${catName}"?`, ()=>deleteCat(catName))} title="Eliminar categoría"
          ...
          {isEdit
            ?<div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
              ...
              <div>
                <label className="lbl">Monto</label>
                <input className="inp" type="number" min="0" value={editData.amount||''}
                  onChange={x=>{const v=x.target.value; setEData(p=>({...p,amount:v})); setEditAmountErr(v!==''&&Number(v)<0?'El monto del gasto no puede ser negativo.':'')}}
                  style={editAmountErr?{borderColor:'var(--red)'}:undefined}/>
                {editAmountErr && <div style={{fontSize:10,color:'var(--red)',marginTop:4,fontWeight:600,maxWidth:160}}>⚠️ {editAmountErr}</div>}
              </div>
...
function TopServices({appts,setTab}) {
  ...
  const now = new Date()
  // Cutoff en formato 'YYYY-MM' usando AÑO/MES locales (no toISOString(),
  // que rolaría al mes anterior cuando el primer día del mes cae cerca de
  // medianoche Bogotá). Maneja underflow de mes con aritmética de meses.
  const ymLocal = (yr, monthZeroBased) => {
    // monthZeroBased puede ser negativo (meses atrás); new Date() normaliza.
    const d = new Date(yr, monthZeroBased, 1)
    return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`
  }
  const cutoff = {
    all:  null,
    '3m': ymLocal(now.getFullYear(), now.getMonth()-2),
    '6m': ymLocal(now.getFullYear(), now.getMonth()-5),
  ...
  const stats = {}
  // Parsea un JSON {"id":precio} guardado en la cita. Si no hay / está corrupto, devuelve {}.
  const parsePrices = raw => {
    if (!raw) return {}
    try { const o = JSON.parse(raw); return o && typeof o === 'object' ? o : {} }
    catch { return {} }
  }
  // Devuelve el precio individual del servicio `id` en la cita `a`, priorizando
  // appt.service_prices (mapa id→precio persistido al crear/editar). Si no existe,
  // cae al reparto proporcional del total (comportamiento legacy para citas viejas).
  const priceAt = (a, id, fallback) => {
    if (!id) return fallback
    const prices = parsePrices(a.servicePrices)
    const v = toN(prices[id])
  ...
  completedAppts.forEach(a => {
    ...
    const perSvcLegacy = names.length ? total/names.length : total
    names.forEach((name,i) => {
      const id = ids[i] || ''
      const revenue = id ? priceAt(a, id, perSvcLegacy) : perSvcLegacy
    ...
    return (
      <div style={{background:'var(--surface)',border:'1.5px solid var(--border)',borderRadius:14,padding:'14px',marginBottom:10}}>
        ...
        <div style={{display:'flex',gap:8,marginTop:12,borderTop:'1px solid var(--border)',paddingTop:10}}>
          {!done && <button className="btn-sm" style={{flex:1}} onClick={()=>setTab('appointments')}>✏️ Editar</button>}
          {!done && <button className="btn-sm" style={{flex:1}}
            onClick={()=>{
